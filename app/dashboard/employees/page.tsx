...
interface Employee {
  ...
  hire_date?: string
  created_at: string
  qr_code_id?: string | null // This will now be the ID from the qr_codes table
}
...
export default function EmployeesPage() {
  ...
  const fetchEmployees = async () => {
    try {
      setLoading(true)
      // Select employees and join with qr_codes to get the qr_code_id
      ...
      // Map the data to include qr_code_id from the joined qr_codes table
      const mappedEmployees = data.map((emp: any) => ({
        ...emp,
        qr_code_id: emp.qr_codes ? emp.qr_codes.id : null,
      }))
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{newEmployeesThisMonth}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    {employees.filter((emp) => emp.qr_code_id).length}
                  </p>
                </div>
